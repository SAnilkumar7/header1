...
// import Mahabhinishkramana from "../assets/Mahabhinishkramana.jpg";
...
// const PLACES = [
//   {
//   name: "The Dawn Of The Buddha's Journey",
//   img: Mahabhinishkramana,
//   desc: "An exquisite limestone relief from Kanaganahalli, Karnataka, depicting Prince Siddhartha Gautama's Great Departure.",
//   details: "This limestone relief, discovered at Kanaganahalli near Sannati in Kalaburagi district, Karnataka, portrays Mahābhinishkramana—the Great Departure of Prince Siddhartha Gautama. Leaving behind his royal life, he began the spiritual journey that led to his enlightenment as Gautama Buddha. Created during the Satavahana–Ikshvaku period (1st–3rd century CE), the sculpture is celebrated as one of the finest masterpieces of early Buddhist art in India."
// },
//   { 
//     name: "Sanchi Stupa", 
//     img: stupa, 
          ...
          {FIGURES.map((f, i) => (
            <div key={f.name} className="grid items-center gap-12 lg:grid-cols-2">
              <Reveal className={i % 2 === 1 ? "lg:order-2" : ""}>
                <div className="relative">
                  <div className="absolute -inset-3 -z-10 rounded-3xl bg-gradient-to-br from-[var(--gold-soft)] to-transparent" />
                  <div className="relative w-full pt-[125%] sm:pt-[100%] overflow-hidden rounded-2xl bg-[var(--royal-deep)]/5 flex items-center justify-center">
                    <img 
                      src={f.img} 
                      alt={f.name} 
                      loading="lazy" 
        ...
        <div className="container-page">
          ...
          <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {PLACES.map((p, i) => (
              <Reveal key={p.name} delay={i * 0.05}>
                <article className="group h-full overflow-hidden rounded-2xl bg-card shadow-sm transition hover:-translate-y-2 hover:shadow-xl">
                  <div className="relative w-full pt-[75%] sm:pt-[56.25%] overflow-hidden bg-[var(--royal-deep)]/5 flex items-center justify-center">
                    <img 
                      src={p.img} 
                      alt={p.name} 
                      loading="lazy" 
                      className="absolute inset-0 w-full h-full object-contain transition duration-700 group-hover:scale-105 p-1 sm:p-2" 
                    />
                  </div>
                  <div className="p-6">
                    <h3 className="text-xl font-semibold">{p.name}</h3>
      ...
      <AnimatePresence>
        {selectedPlace && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] overflow-y-auto bg-[var(--royal-deep)]/90 p-4 flex items-center justify-center"
            onClick={() => setSelectedPlace(null)}
          >
            <motion.div
              initial={{ scale: 0.9, opacity: 0, y: 20 }}
            ...
            >
              ...
              {/* Modal Image Container - Full width with auto height */}
              <div className="w-full bg-[var(--royal-deep)]/5 flex items-center justify-center py-8 px-4 sm:px-8">
                <img 
                  src={selectedPlace.img} 
                  alt={selectedPlace.name} 
